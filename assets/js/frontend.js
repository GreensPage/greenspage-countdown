...
( function () {
	...
	/**
	 * Split a duration across only the units that are actually on screen.
	 * Hiding "days" must roll those days into hours, not silently drop them.
	 */
	...
	function humanise( parts, units ) {
		...
		ORDER.forEach( function ( unit ) {
			if ( units.indexOf( unit ) === -1 || typeof parts[ unit ] === 'undefined' ) {
				return;
			}

			var value = parts[ unit ];

			// Reading "0 hours" aloud helps nobody.
			if ( 0 === value ) {
				return;
			}

			// Seconds only get spoken when nothing larger is left.
			if ( 'seconds' === unit && bits.length ) {
				return;
			}
	...
	function Countdown( root ) {
		...
		try {
			config = JSON.parse( root.getAttribute( 'data-gpcd' ) || '{}' );
		} catch ( e ) {
			return;
		}
		...
		if ( 'evergreen' === config.mode ) {
			...
			if ( ! first ) {
				first = now;
				writeStore( key, first );
			} else if ( config.resetDays > 0 && now - first > config.resetDays * 86400000 ) {
				// The offer window has lapsed long enough that this visitor gets a fresh run.
				first = now;
				writeStore( key, first );
			}
		...
		function finish() {
			...
			if ( 'message' === config.onExpire ) {
				...
				if ( expired ) {
					...
					if ( sr ) {
						sr.textContent = expired.textContent.trim();
					}
				}
				return;
			}

			if ( 'redirect' === config.onExpire && config.redirect ) {
				window.location.replace( config.redirect );
				return;
			}

			if ( sr ) {
				sr.textContent = 'Countdown finished';
			}
		}
		...
		function tick() {
			...
			ORDER.forEach( function ( unit ) {
				if ( nums[ unit ] && typeof parts[ unit ] !== 'undefined' ) {
					...
					if ( nums[ unit ].textContent !== next ) {
						nums[ unit ].textContent = next;
					}
				}
			} );
			...
			if ( sr ) {
				// Announce when the wording actually changes, and never more than
				// once every ten seconds. Screen readers should not tick.
		...
		// Coming back to a backgrounded tab should not show a stale time.
